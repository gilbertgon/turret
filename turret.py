...
class Turret(object):
            ...
            try:
                while True:
                    ...
                    if not ch:
                        ...
                        if MOTOR_X_REVERSED:
                            Turret.move_backward(kit.motor1, .5)
                        else:
                            Turret.move_forward(kit.motor1, .5)
                    elif ch == "d":
                        if MOTOR_X_REVERSED:
                            Turret.move_forward(kit.motor1, .5)
                        else:
        ...
        with raw_mode(sys.stdin):
            try:
                while True:
                    ...
                    if ch == "w":
                        if MOTOR_Y_REVERSED:
                            Turret.move_forward(kit.motor2, .5)
                        else:
                            Turret.move_backward(kit.motor2, .5)
                    elif ch == "s":
                        if MOTOR_Y_REVERSED:
                            Turret.move_backward(kit.motor2, .5)
                        else:
    ...
    def __move_axis(self, contour, frame):
        ...
        if (target_steps_x - self.current_x_steps) > 0:
            ...
            if MOTOR_X_REVERSED:
                t_x = threading.Thread(target=Turret.move_forward, args=(kit.motor1, .5,))
            else:
                t_x = threading.Thread(target=Turret.move_backward, args=(kit.motor1, .5,))
        ...
        if (target_steps_y - self.current_y_steps) > 0:
            ...
            if MOTOR_Y_REVERSED:
                t_y = threading.Thread(target=Turret.move_backward, args=(kit.motor2, .5,))
            else:
                t_y = threading.Thread(target=Turret.move_forward, args=(kit.motor2, .5,))
